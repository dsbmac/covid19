...
export default function WorldSummaryTable(props) {
  const classes = useStyles()
  const { data: summaryData } = props
  const { Date: timeStamp, Countries: data = [] } = summaryData
  ...
  const isSelected = (Country) => selected.indexOf(Country) !== -1

  const emptyRows =
    rowsPerPage - Math.min(rowsPerPage, data.length - page * rowsPerPage)

  const title = `${timeStamp} Case Summary`
  return (
